...
- `directory` string optional
  ...
  - Root folder of static assets to be served. Refer to [Assets](/workers/configuration/assets/#configuration)for more details.
...
- `outbound` object optional
  ...
  - `service` string required The name of the [outbound Worker](/cloudflare-for-platforms/workers-for-platforms/configuration/outbound-workers/) to bind to.
...
The [assets binding](/workers/configuration/assets/) allows developers to run frontend websites on Workers. It provides asset hosting and routing configuration options.

This binding is limited to one set of assets per Worker project.

To bind assets to your Worker, assign an array of the below object to the `assets` key.
...
- `html_handling`: `'auto-trailing-slash' | 'force-trailing-slash' | 'drop-trailing-slash' | 'none'` optional

  - Determines redirects/rewrites of requests for HTML content. If unspecified, the default mode is `'auto-trailing-slash'`. Learn more [here](/workers/configuration/assets/#how-it-works).

- `not_found_handling`: `'single-page-application' | '404-page' | 'none'` optional

  - Determines the handling of requests that do not map to an asset. If unspecified, the default mode is `'not_found_handling'`. Learn more [here](/workers/configuration/assets/#how-it-works).
